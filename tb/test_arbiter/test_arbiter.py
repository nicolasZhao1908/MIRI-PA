...
async def test_with_random_suit(dut):
    ...
    for i in range(n * 3):
        ...
        dut.word_1.value = suit1[index1][3]
        ...
        dut.word_2.value = suit2[index2][3]
        ...
        if dut.hit_1.value or suit1[index1][0] < 0:
            if suit1[index1][0] == 0 and dut.hit_1.value:
                if suit1[index1][3] == 0:
                    py_data = m.load_ONLY_ONE_BYTE(suit1[index1][1])
                else:
                    py_data = m.load_ONLY_ONE_WORD(suit1[index1][1])
                ...
                m.store(suit1[index1][1], suit1[index1][2], suit1[index1][3])
                executed.append(suit1[index1])
                # print("STORED! 1")
                #
                # print("P2")
        ...
        if dut.hit_2.value or suit2[index2][0] < 0:
            if suit2[index2][0] == 0 and dut.hit_2.value:
                if suit2[index2][3] == 0:
                    py_data = m.load_ONLY_ONE_BYTE(suit2[index2][1])
                else:
                    py_data = m.load_ONLY_ONE_WORD(suit2[index2][1])
                ...
                m.store(suit2[index2][1], suit2[index2][2], suit2[index2][3])
                executed.append(suit2[index2])
                # print("STORED! 2")
...
def create_random_suit(n,suit_nr):
    ...
    for _ in range(n):
        if (random.randint(0, 1) == 0):
            suit.append([random.randint(-3, 1), random.randint(0 + 16 * 4 * suit_nr + suit_nr, 16 * 4 * (suit_nr + 1) + suit_nr), random.randint(0, 255), 0])
        else:
            suit.append([random.randint(-3, 1), random.randint(0 + 16 * suit_nr + suit_nr, 16 * (suit_nr + 1) + suit_nr) * 4, random.randint(0, 4096 * 2), 1])
    return suit
